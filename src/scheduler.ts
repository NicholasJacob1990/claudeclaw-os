...
/** Max time (ms) a mission task can run before being killed (scheduled tasks share this floor). */
...
export function initScheduler(send: Sender, agentId = 'main'): void {
  ...
  // 30s when the queue is idle.
  scheduleNextMissionPoll(MISSION_POLL_MIN_MS);
  ...
  for (const task of tasks) {
    ...
    messageQueue.enqueue(chatId, async () => {
      ...
      try {
        ...
        // Send raw text — sender (sendToPrimary) extracts [SEND_FILE]/[SEND_PHOTO]
        // markers, dispatches attachments, then formats/splits the remaining text.
        await sender(text);
